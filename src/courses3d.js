...
export const createCourseLayer = (scene, bounds, groups, reduceMotion) => {
  ...
  let active = null; // {line, pins[], prefectures[], total, progress}
  ...
  const clear = () => {
    if (!active) return;
    for (const g of active.prefectures) {
  ...
  const show = (course) => {
    ...
    const prefectures = isos.map((iso) => byId.get(iso)).filter(Boolean);
    for (const g of prefectures) g.userData.targetY = 4;
