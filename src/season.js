  ...
  const apply = (season) => {
    ...
    for (let i = 0; i < c.n; i++) {
      const mat = new THREE.SpriteMaterial({
        map: tex, transparent: true, depthWrite: false, opacity: c.opacity,
  ...
  const update = (dt) => {
    ...
    if (current === "summer") {
      ...
      for (const it of items) {
        const k = ((t * 0.34 * it.speed + it.phase) % (Math.PI * 2)) / (Math.PI * 2);
        const grow = Math.min(1, k * 3.2);
        it.sp.scale.setScalar(it.base * (0.25 + grow * 0.85));
        it.sp.material.opacity = c.opacity * Math.max(0, 1 - k * 1.25) * (k > 0.02 ? 1 : 0);
      }
      return;
    }
    const top = span * 0.42;
